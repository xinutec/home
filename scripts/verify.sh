#!/usr/bin/env bash
# home verify — wraps `pnpm run verify` (backend tsc + biome + vitest, frontend
...
# `pnpm run verify` above is tsc/lint/vitest only (no ng build) — and per
...
nix develop -c bash -c '
  ...
  ( cd frontend && pnpm exec ng build && pnpm run ui-check )
'
