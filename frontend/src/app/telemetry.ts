...
export function labelFor(node: EventTarget | null): string | null {
	...
	// "thermostatClimate"; and aria-hidden content is by definition not part of what the
 ...
 *
 * **Sign-in gates this, unlike every read on this host.** home is publicly
 * readable; an ungated write would be an open log-write on the internet. A
 * signed-out visitor's batch is simply refused, and the dashboard is unchanged
 * for them. This reports *activity*: a tap that hits a cache, a control
 ...
 *
 * Uses `fetch` directly rather than `HttpClient` so a failing request cannot
 * generate telemetry about itself through the interceptor chain.
 *
